(() => {
  if ((window.scc ??= {}).actualites) return;
  window.scc.actualites = true;
  ...
  const init = () => {
    ...
    const une = section.querySelector('.news-featured');
    if (une && !une.querySelector('.w-dyn-item')) une.remove();

    const dater = (racine) => {
      for (const item of racine.querySelectorAll('.w-dyn-item')) {
    ...
    const grille = section.querySelector('.article-grid');
    if (grille) new MutationObserver(() => dater(grille)).observe(grille, { childList: true });

    for (const pilule of section.querySelectorAll('.news-cats .news-cat')) {
      const radio = pilule.querySelector('input[type="radio"]');
      const etiquette = pilule.querySelector('.news-cat-label');
      if (!radio || !etiquette) continue;
      const valeur = etiquette.textContent.trim();
      radio.setAttribute('fs-list-field', 'categorie');
      radio.setAttribute('fs-list-value', valeur);
      radio.value = valeur;
    }

    const tout = section.querySelector('[data-actus-tout]');
    if (!tout) return;

    const majTout = () => {
      const actif = section.querySelector('.news-cats input[type="radio"]:checked');
