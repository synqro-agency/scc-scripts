(() => {
  if ((window.scc ??= {}).ficheActualite) return;
  window.scc.ficheActualite = true;
  ...
  const dansLaFiche = (debut, fin, aujourdhui) => {
    if (!debut) return '';
    const bout = finUtile(debut, fin);
    if (!bout) return `${JOURS[debut.getDay()]} ${avecAn(debut, aujourdhui)}`;
    if (memeMois(debut, bout)) return `${numero(debut)} \u2192 ${avecAn(bout, aujourdhui)}`;
    if (memeAn(debut, bout)) return `${jourMois(debut)} \u2192 ${avecAn(bout, aujourdhui)}`;
    return `${jourMoisAn(debut)} \u2192 ${jourMoisAn(bout)}`;
  };
  ...
  const init = () => {
    ...
    if (aside) {
      const texte = dansLaFiche(...bornes(aside), aujourdhui);
      const brut = brutDe(aside, 'debut');
      if (texte || brut) ecrire(aside.querySelector('[data-date-fiche]'), texte, brut);
      else aside.querySelector('[data-ligne="date"]')?.remove();
    }

    const section = document.querySelector('.news-related-section');
    if (!section) return;
    const items = [...(section.querySelector('.article-grid')?.children ?? [])];
